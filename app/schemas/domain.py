...
class RiskAssessedClause(ExtractedClause):
    ...
    cross_references: List[str] = Field(default_factory=list)  # Tool names used in agent_trace

    # Override confidence if the risk agent adjusts it
    confidence: float

    # True if the Agent Path produced a label different from DeBERTa's preliminary label
...
class RiskReportRecommendation(BaseModel):
    clause_id: str
    clause_type: str = ""           # e.g. "Non-Compete", "Cap On Liability"
    page_no: Optional[str] = None
    content_label: Optional[str] = None
    explanation: str
    recommendation: str
