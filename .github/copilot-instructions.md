...
- Dataset: CUAD SQuAD-format from HuggingFace (`kenlevine/CUAD`)
...
- Data contracts (ClauseObject etc.) live in `src/common/schema.py`
...
- Synthetic labels generated by Qwen-32B (primary), Gemini/OpenAI (backup)
- When generating LangGraph code, use StateGraph with typed state dict
- Data contracts (RiskAssessedClause etc.) live in `src/common/schema.py`
